...
class OrdercloudAPI extends RESTDataSource {
	...
	async getAuthToken() {
		...
		const authBody =
			`client_id=${clientID}&grant_type=client_credentials&client_secret=${clientSecret}&scope=${roles.join(" ")}`;
	...
	async getPriceForProduct(productID) {
		...
		if (Array.isArray(priceSchedule.PriceBreaks) && priceSchedule.PriceBreaks.length > 0) {
